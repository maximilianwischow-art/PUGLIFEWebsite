...
function knockNearBlackCorners(data, width, height) {
  // Soften leftover solid black studio backgrounds that generators sometimes emit.
  for (let i = 0; i < data.length; i += 4) {
    const r = data[i];
    const g = data[i + 1];
    const b = data[i + 2];
  ...
  // Keep content; only clear pure black pixels (common AI bg).
  void width;
  void height;
}
...
async function processOne(srcName, outName) {
  ...
  knockMagenta(data);
  knockNearBlackCorners(data, info.width, info.height);
  ...
  console.log(`wrote gear/${outName}`);
}

async function main() {
  await mkdir(outDir, { recursive: true });
  const files = await readdir(srcDir);
  const gearFiles = files.filter((f) => /^gear-.+\.png$/i.test(f));
  for (const file of gearFiles) {
    const base = file.replace(/^gear-/i, "").replace(/\.png$/i, "");
    // Skip bare class files already handled by process-tavern-gear.mjs unless missing.
    await processOne(file, `${base}.webp`);
  }
}
