/**
 * Regenerate data/heart-of-darkness-craftables-fallback.json when Wowhead markup changes.
 * Usage:
 *   curl.exe -sS -A "Mozilla/5.0" "https://www.wowhead.com/tbc/item=32428/heart-of-darkness#reagent-for" -o tmp-hod.html
 *   node scripts/gen-heart-of-darkness-fallback.mjs
 */
...
function hodCountFromWowheadReagents(reagents) {
  const pairs = Array.isArray(reagents) ? reagents : [];
  for (const p of pairs) {
    if (Array.isArray(p) && p.length >= 2 && Number(p[0]) === HEART_OF_DARKNESS_WOW_ITEM_ID) {
      const n = Number(p[1]);
...
function parseHtml(html) {
  ...
  return spellRows
    .map((row) => {
      ...
      const hodNeeded = hodCountFromWowheadReagents(row?.reagents);
      const profession = professionFromWowheadSpellRow(row);
      return { itemID, itemName, profession, hodNeeded };
    })
    .filter((row) => row.itemID > 0 && row.itemName)
    .sort((a, b) => a.itemName.localeCompare(b.itemName));
}

const html = readFileSync(join(root, "tmp-hod.html"), "utf8");
const items = parseHtml(html);
const outPath = join(root, "data", "heart-of-darkness-craftables-fallback.json");
writeFileSync(outPath, JSON.stringify({ generatedAt: Date.now(), source: "wowhead-tbc-item-32428", items }, null, 2));
