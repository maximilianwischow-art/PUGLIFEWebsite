...
const wipeMeta = document.querySelector("#wipeMeta");
const wipeHeatmap = document.querySelector("#wipeHeatmap");
...
function fmtDateTime(timestampMs) {
  if (!timestampMs) return "-";
  const date = new Date(Number(timestampMs));
  return Number.isNaN(date.getTime()) ? "-" : date.toLocaleString();
}

async function loadWipeHeatmap() {
  try {
    const response = await fetch(`/api/wcl/guild/${GUILD_ID}/wipe-heatmap?limit=50`);
    const payload = await response.json();
    if (!response.ok) {
      throw new Error(payload.error || "Failed to fetch wipe heatmap");
    }
    const rows = payload?.heatmap || [];
    wipeMeta.textContent = `${rows.length} tracked boss(es) across recent raids.`;
    if (!rows.length) {
      wipeHeatmap.innerHTML = "<div>No wipe data yet.</div>";
      return;
    }

    wipeHeatmap.innerHTML = rows
      .slice(0, 12)
      .map((row) => {
        const severity = row.wipeRate >= 60 ? "high" : row.wipeRate >= 30 ? "mid" : "low";
        const avgWipe = row.avgWipeMs ? fmtDuration(row.avgWipeMs) : "-";
        return `
          <div class="heatmap-row ${severity}">
            ...
            <div class="heatmap-stats">
              <span>${row.wipes}/${row.attempts}</span>
              <span>${row.wipeRate.toFixed(0)}%</span>
              <span>${avgWipe}</span>
            </div>
          </div>
        `;
    ...
    wipeMeta.textContent = error.message || "Unable to load wipe heatmap.";
    wipeHeatmap.innerHTML = "<div>Could not load wipe data.</div>";
  }
}
