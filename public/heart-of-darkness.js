...
function entryHeartOfDarknessTotal(row) {
  const items = Array.isArray(row?.items) ? row.items : [];
  return items.reduce((sum, it) => sum + hodNeeded(it?.hodNeeded), 0);
}

function hodNeeded(v) {
  const x = Number(v);
...
function compareDemandRowsOpenFirst(a, b) {
  ...
  return entryHeartOfDarknessTotal(b) - entryHeartOfDarknessTotal(a);
}

const P3_DEMAND_DONE_ICON =
...
function p3DemandDoneStatusHtml() {
  return `<span class="p2-demand-status is-done" title="Marked done by raid lead">${P3_DEMAND_DONE_ICON}<span>Done</span></span>`;
}

function renderP3DemandStats(total, done, open) {
  const el = document.getElementById("p3DemandStats");
  if (!el) return;
  if (total <= 0) {
    el.hidden = true;
    el.innerHTML = "";
  ...
  el.innerHTML = `
    <div class="p2-demand-stat p2-demand-stat--total">
      ...
      <span class="p2-demand-stat-value">${total}<span class="p2-demand-stat-unit"> HoD</span></span>
    </div>
    <div class="p2-demand-stat p2-demand-stat--open">
      <span class="p2-demand-stat-label">Open</span>
      <span class="p2-demand-stat-value">${open}<span class="p2-demand-stat-unit"> HoD</span></span>
    </div>
    <div class="p2-demand-stat p2-demand-stat--done">
      <span class="p2-demand-stat-label">Done</span>
      <span class="p2-demand-stat-value">${done}<span class="p2-demand-stat-unit"> HoD</span></span>
    </div>
  `;
}
...
function applyCraftableHodCounts(items) {
  if (!Array.isArray(items) || !craftables.length) {
    return Array.isArray(items) ? items.map((row) => ({ ...row, hodNeeded: hodNeeded(row?.hodNeeded) })) : [];
  }
  return items.map((row) => {
    const id = canonicalItemId(row.itemName, row.itemID);
    const c = craftables.find((x) => n(x.itemID) === id);
    if (c && Number(c.hodNeeded) > 0) {
      return { ...row, hodNeeded: hodNeeded(c.hodNeeded) };
    }
    return { ...row, hodNeeded: hodNeeded(row?.hodNeeded) };
  });
}
...
function renderHodCost(cost, opts = {}) {
  const compact = Boolean(opts.compact);
  const label = compact ? `${n(cost)} HoD` : `${n(cost)} Heart of Darkness`;
  ...
  const hodIconSrc = normalizeItemIconUrl(heartOfDarknessIcon);
  const icon = hodIconSrc
    ? `<img class="vortex-inline-icon" src="${esc(hodIconSrc)}" alt="" width="18" height="18" loading="lazy" decoding="async" referrerpolicy="no-referrer" />`
    : "";
...
function updateCraftableOptions() {
  const search = document.getElementById("hodCraftableSearch");
  const select = document.getElementById("hodCraftableSelect");
  if (!select) return;
  ...
  select.innerHTML = [
    ...
    ...rows.map((row) => {
      const nv = hodNeeded(row.hodNeeded);
      return `<option value="${n(row.itemID)}">${esc(row.itemName)}${row.profession ? ` (${esc(row.profession)})` : ""} — ${nv} HoD</option>`;
    }),
  ].join("");
...
function renderSelectedItems() {
  const host = document.getElementById("hodSelectedItems");
  if (!host) return;
  ...
  host.innerHTML = selectedItems
      ...
      (row, idx) =>
        `<span class="loot-recipient-pill">
          <span class="vortex-selected-item-main">
            ...
            <span class="vortex-cost-wrap" title="Per craft — from TBC recipe data (Heart of Darkness count).">
            ${renderHodCost(hodNeeded(row.hodNeeded), { compact: true })}
            </span>
          </span>
          ${
            hodCanEdit
    ...
    .join("");
  window.WowItemTooltip.bindLootTooltipHandlers(document.getElementById("hodSelectedItems"), (id) =>
    itemMetaById.get(Number(id))
  );
}

async function loadCraftables() {
  const payload = await getJson("/api/heart-of-darkness/craftables");
  ...
  if (HEART_OF_DARKNESS_ITEM_ID > 0) ids.push(HEART_OF_DARKNESS_ITEM_ID);
  ...
  heartOfDarknessIcon = normalizeItemIconUrl(String(itemMetaById.get(HEART_OF_DARKNESS_ITEM_ID)?.icon || "").trim());
}
...
function setCharacterRole(role, { syncButtons = true } = {}) {
  ...
  document.getElementById("hodRoleMain")?.classList.toggle("is-active", selectedCharacterRole === "main");
  document.getElementById("hodRoleAlt")?.classList.toggle("is-active", selectedCharacterRole === "alt");
}

function syncCustomCharacterVisibility() {
  const select = document.getElementById("hodCharacterSelect");
  const custom = document.getElementById("hodCharacterCustom");
 ...
 * use `rowspan` on the Raider / Total HoD / Updated cells so the Item column
...
function buildDemandRowsForRaider(row) {
  ...
  const totalNv = n(entryHeartOfDarknessTotal(row));
  ...
  return items
    .map((it, idx) => {
      ...
      const statusHtml = checked ? p3DemandDoneStatusHtml() : "";
      cells.push(
...
function refreshDemandTable() {
  const tbody = document.getElementById("hodDemandTbody");
  const tfoot = document.getElementById("hodDemandTfoot");
  const wrap = document.getElementById("hodDemandWrap");
  const empty = document.getElementById("hodDemandEmpty");
  if (!tbody) return;
  ...
  const q = document.getElementById("hodDemandSearch")?.value || "";
  const rows = all
  ...
  const grandTotal = rows.reduce((sum, row) => sum + entryHeartOfDarknessTotal(row), 0);
  let checkedHod = 0;
  for (const row of rows) {
    for (const it of row.items || []) {
      if (isDemandItemChecked(row, Math.max(0, Math.floor(Number(it.itemID) || 0)))) {
        checkedHod += hodNeeded(it?.hodNeeded);
      }
    }
  }
  const openHod = Math.max(0, grandTotal - checkedHod);
  renderP3DemandStats(grandTotal, checkedHod, openHod);
  const meta = document.getElementById("hodMeta");
  if (meta) {
    meta.textContent = `Total guild need: ${grandTotal} Heart of Darkness · Done: ${checkedHod} · Open: ${openHod}`;
...
function ensureDemandFilterListeners() {
  ...
  const search = document.getElementById("hodDemandSearch");
...
function mapEntryItemsToSelected(entry) {
  ...
  return applyCraftableHodCounts(
    entry.items.map((row) => {
      const id = canonicalItemId(row.itemName, row?.itemID);
      ...
      return {
        ...
        hodNeeded: hodNeeded(row?.hodNeeded),
      };
    })
  );
}
...
async function loadTracker() {
  const meta = document.getElementById("hodMeta");
  const saveBtn = document.getElementById("hodSaveBtn");
  const search = document.getElementById("hodCraftableSearch");
  const select = document.getElementById("hodCraftableSelect");
  const addBtn = document.getElementById("hodAddItemBtn");
  try {
  ...
  const payload = await getJson("/api/heart-of-darkness/needs");
  ...
  const checkedHod = n(payload?.checkedHod);
  const openHod = n(payload?.openHod ?? Math.max(0, total - checkedHod));
  renderP3DemandStats(total, checkedHod, openHod);
  if (meta) {
    meta.textContent = `Total guild need: ${total} Heart of Darkness · Done: ${checkedHod} · Open: ${openHod}`;
  ...
  const charSelect = document.getElementById("hodCharacterSelect");
  const charCustom = document.getElementById("hodCharacterCustom");
  if (charSelect) charSelect.disabled = false;
  if (charCustom) charCustom.disabled = false;
  document.getElementById("hodRoleMain")?.removeAttribute("disabled");
  document.getElementById("hodRoleAlt")?.removeAttribute("disabled");
}

document.getElementById("hodSaveBtn")?.addEventListener("click", async () => {
  const btn = document.getElementById("hodSaveBtn");
  if (!btn) return;
  if (!hodCanEdit) {
  ...
  try {
    await getJson("/api/heart-of-darkness/needs/my", {
      method: "PUT",
...
document.getElementById("hodCharacterSelect")?.addEventListener("change", () => {
  const select = document.getElementById("hodCharacterSelect");
  syncCustomCharacterVisibility();
  if (!select) return;
...
document.getElementById("hodCharacterCustom")?.addEventListener("change", () => {
  const { name } = getSelectedRequestCharacter();
...
document.getElementById("hodRoleMain")?.addEventListener("click", () => setCharacterRole("main"));
document.getElementById("hodRoleAlt")?.addEventListener("click", () => setCharacterRole("alt"));

document.getElementById("hodAddItemBtn")?.addEventListener("click", () => {
  const select = document.getElementById("hodCraftableSelect");
  if (!select) return;
  ...
  selectedItems.push({
    ...
    hodNeeded: hodNeeded(picked.hodNeeded),
  });
  renderSelectedItems();
});

document.getElementById("hodCraftableSearch")?.addEventListener("input", () => {
  updateCraftableOptions();
});
