...
export const RACES = [
  ...
  { id: "gnome", name: "Gnome", faction: "alliance", portraitKey: "gnome" },
  {
    ...
    isNewRace: true,
    note: "Neutral race. Start on Zephras Isle. On this guild site Forever rolls lock Alliance Skyborne.",
  },
...
export function validateWowForeverPick(body) {
  ...
  const faction = resolvedFaction(raceId, "alliance");
  if (!faction || faction !== "alliance") {
    return { ok: false, error: "PUG LIFE BALANCE Forever rolls are Alliance only." };
  }
  if (race.faction === "horde") {
    return { ok: false, error: "PUG LIFE BALANCE Forever rolls are Alliance only." };
  }
