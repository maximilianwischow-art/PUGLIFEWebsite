...
export const IMPORTANT_DEBUFFS = Object.freeze([
  ...
  },
  {
    ...
    /** Boss debuff on WCL is "Hunter's Mark" (rank 5 in TBC); 19425 is the passive talent, not the aura. */
    spellId: 14325,
    spellIds: [14325, 14324, 14323, 14322, 14321],
    name: "Improved Hunter's Mark",
    wclNames: ["Hunter's Mark"],
    appliedBy: "Hunter",
    category: "attack",
    description:
      "Hunter's Mark on the boss (110 ranged AP; +25% with 3/3 Improved Hunter's Mark). WCL tracks uptime as Hunter's Mark.",
  },
  {
    key: "thunder-clap",
    spellId: 25264,
...
function catalogEntryNameKeys(catalogRow) {
  const keys = new Set();
  const add = (label) => {
  ...
  add(catalogRow?.name);
  for (const alt of catalogRow?.wclNames || catalogRow?.abilityNames || []) add(alt);
  return keys;
}
...
export function matchImportantDebuffFromEntry(entry) {
  ...
  return (
    IMPORTANT_DEBUFFS.find((row) => {
      for (const rowKey of catalogEntryNameKeys(row)) {
        if (rowKey === nameKey || nameKey.includes(rowKey) || rowKey.includes(nameKey)) return true;
      }
      return false;
    }) || null
  );
