...
/**
 * Sum per-report usage rows into one guild-wide leaderboard payload.
 * @param {Array<{ players?: object[], fightsScanned?: number, totalEvents?: number }>} parts
 */
export function mergeConsumablesUsageResults(parts) {
  /** @type {Map<string, Record<string, number>>} */
  const byPlayer = new Map();
  let fightsScanned = 0;
  let totalEvents = 0;
  ...
  for (const part of Array.isArray(parts) ? parts : []) {
    ...
    reportsScanned += 1;
    fightsScanned += Number(part.fightsScanned || 0);
    totalEvents += Number(part.totalEvents || 0);
    for (const player of Array.isArray(part.players) ? part.players : []) {
      const name = String(player?.name || "").trim();
      if (!name) continue;
      if (!byPlayer.has(name)) byPlayer.set(name, emptyCounts());
      const counts = byPlayer.get(name);
      for (const row of TBC_USAGE_CONSUMABLES) {
        counts[row.key] = (counts[row.key] || 0) + Number(player.counts?.[row.key] || 0);
      }
    }
  }
...
/**
 * Keep only leaderboard-tracked consumables (potions, scrolls, runes).
 * @param {{ players?: object[], catalog?: object[], rosterCount?: number, fightsScanned?: number, totalEvents?: number, reportsScanned?: number }} merged
 */
export function applyLeaderboardUsageFilter(merged) {
  const catalog = leaderboardUsageConsumableCatalogForApi();
  const players = (Array.isArray(merged?.players) ? merged.players : [])
    .map((player) => {
      const counts = Object.fromEntries(
        TBC_LEADERBOARD_USAGE_KEYS.map((key) => [key, Number(player.counts?.[key] || 0)])
      );
      const totalUses = Object.values(counts).reduce((sum, n) => sum + Number(n || 0), 0);
      return { name: player.name, counts, totalUses };
    })
    .sort((a, b) => {
      if (b.totalUses !== a.totalUses) return b.totalUses - a.totalUses;
      return String(a.name).localeCompare(String(b.name));
  ...
  return {
    players,
    rosterCount: players.length,
    fightsScanned: Number(merged?.fightsScanned || 0),
    catalog,
    totalEvents: Number(merged?.totalEvents || 0),
    reportsScanned: Number(merged?.reportsScanned || 0),
  };
}
