...
export function buildDebuffEventsFilter() {
  ...
  const quoted = [...names].map((name) => `'${escapeWclFilterString(name)}'`);
  return `type in ('applydebuff','refreshdebuff') and ability.name in (${quoted.join(",")})`;
}
...
function debuffTargetFilter(catalogRow) {
  ...
  if (!names.length) return "ability.name = ''";
  if (names.length === 1) return `ability.name = '${escapeWclFilterString(names[0])}'`;
  const quoted = names.map((name) => `'${escapeWclFilterString(name)}'`).join(", ");
