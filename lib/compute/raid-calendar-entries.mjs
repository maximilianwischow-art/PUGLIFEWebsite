...
export function isAmbiguousCombinedRaidZone(zoneRaw) {
  ...
  const hasHyjal = z.includes("hyjal") || z.includes("mount hyjal");
  const hasBt = z.includes("black temple") || /\bbt\b/.test(z);
  if (hasHyjal && hasBt) return true;
  return false;
}
...
export function resolveTrackedRaidZoneName(zoneRaw) {
  ...
  if (z.includes("hyjal") || z.includes("mount hyjal")) return "Hyjal Summit";
  if (z.includes("black temple") || z === "bt") return "Black Temple";
  return null;
}
