/**
 * Self-contained class + spec (+ optional Classic Armory GearScore) resolver
 * used by both the in-server `runSyncCharacterSpecs` worker and the standalone
 ...
 *   4. ClassicArmory character + talent APIs (last-resort public fallback)
 *
 * When `includeClassicArmoryGearScore` is true, the resolver also requests
 * Classic Armory once when class/spec were already filled from Battle.net /
 * Raider.IO so `gearScore` can still be populated.
 *
...
async function postJson(url, body) {
  const res = await fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "fallen-tacticians-api/1.0 (+character-specs sync)",
    },
...
export async function fetchClassicArmoryCharacter({ baseUrl, region, flavor, realmSlug, characterName }) {
  return postJson(`${String(baseUrl || "").replace(/\/+$/, "")}/api/v1/character`, {
    region,
    realm: realmSlug,
    name: characterName,
    flavor,
...
async function fetchClassicArmoryTalents({ baseUrl, region, flavor, realmSlug, characterName, classId }) {
  if (!classId) return null;
  return postJson(`${String(baseUrl || "").replace(/\/+$/, "")}/api/v1/character/talents`, {
    region,
    realm: realmSlug,
    name: characterName,
    flavor,
...
function classFromClassicArmoryCharacter(data) {
...
function classIdFromClassicArmoryCharacter(data) {
  const character = data?.character && typeof data.character === "object" ? data.character : data;
  const id = Number(character?.class_id ?? character?.classId);
  return Number.isInteger(id) && id > 0 ? id : null;
}

function specFromClassicArmoryPayload(characterData, talentsData) {
  const character = characterData?.character && typeof characterData.character === "object" ? characterData.character : characterData;
  const direct = String(
  ...
  const talents = talentsData?.talents && typeof talentsData.talents === "object" ? talentsData.talents : talentsData;
  const activeIndex = Number(talents?.activeIndex || 0);
  const trees = Array.isArray(talents?.trees) ? talents.trees : [];
  const activeTree = trees[Number.isInteger(activeIndex) && activeIndex >= 0 ? activeIndex : 0] || null;
  return String(activeTree?.specialization_name || activeTree?.specializationName || "").trim() || null;
}

/**
 * Best-effort GearScore from Classic Armory `POST /api/v1/character` JSON.
 */
export function gearScoreFromClassicArmoryCharacter(data) {
  if (!data || typeof data !== "object") return null;
  const character = data.character && typeof data.character === "object" ? data.character : data;
  const candidates = [
...
/**
 * Public character page URL on classic-armory.org.
 */
export function classicArmoryCharacterPageUrl({
  publicBaseUrl = "https://classic-armory.org",
  region,
  flavor,
  realmSlug,
  characterName,
  ...
  const r = String(region || "eu").trim().toLowerCase() || "eu";
  const f = String(flavor || "tbc-anniversary").trim() || "tbc-anniversary";
  const rs = String(realmSlug || "").trim().toLowerCase();
  const name = String(characterName || "").trim();
  if (!base || !rs || !name) return null;
  const enc = (s) => encodeURIComponent(s);
  return `${base}/character/${enc(r)}/${enc(f)}/${enc(rs)}/${enc(name)}`;
}

/**
 * Build a resolver bound to the given configuration. Returns an async
 * `resolve({ characterName, realm })` that produces
 * `{ wowClass, wowSpec, source, gearScore, gearScoreSource }` (any field may be `null`).
 *
 * @param {{
 ...
 *   classicArmoryApiBase?: string,
 *   classicArmoryFlavor?: string,
 *   defaultRealm?: string,
 *   includeClassicArmoryGearScore?: boolean,
 * }} config
 */
export function createCharacterSpecResolver(config) {
  const {
    ...
    raiderIoApiBase,
    raiderIoRegion,
    classicArmoryApiBase = "https://classic-armory.org",
    classicArmoryFlavor = "tbc-anniversary",
    defaultRealm,
    includeClassicArmoryGearScore = false,
  } = config || {};
  ...
  return async function resolve({ characterName, realm } = {}) {
    const empty = { wowClass: null, wowSpec: null, source: null, gearScore: null, gearScoreSource: null };
    const name = String(characterName || "").trim();
    if (!name) return empty;
    const realmRaw = String(realm || "").trim() || String(defaultRealm || "").trim();
    if (!realmRaw) return empty;
    ...
    let source = null;
    let gearScore = null;
    let gearScoreSource = null;
    let armoryPayloadForGear = null;
    ...
    if (!wowClass || !wowSpec) {
      let armoryCharacter = null;
      try {
        armoryCharacter = await fetchClassicArmoryCharacter({
          baseUrl: classicArmoryApiBase,
          region: raiderIoRegion || blizzardRegion || "eu",
          flavor: classicArmoryFlavor,
          realmSlug: slug,
          characterName: name,
        });
      } catch {
        armoryCharacter = null;
      }
      if (armoryCharacter) armoryPayloadForGear = armoryCharacter;
      if (armoryCharacter) {
        if (!wowClass) {
          const c = classFromClassicArmoryCharacter(armoryCharacter);
          const display = c ? normalizeClassDisplayName(c) : "";
          if (display) {
            wowClass = display;
            source = source || "classic-armory";
          }
        }
        if (!wowSpec) {
          let armoryTalents = null;
          try {
            armoryTalents = await fetchClassicArmoryTalents({
              baseUrl: classicArmoryApiBase,
              region: raiderIoRegion || blizzardRegion || "eu",
              flavor: classicArmoryFlavor,
              realmSlug: slug,
              characterName: name,
              classId: classIdFromClassicArmoryCharacter(armoryCharacter),
            });
          } catch {
            armoryTalents = null;
          }
          const s = specFromClassicArmoryPayload(armoryCharacter, armoryTalents);
          const cleaned = s ? normalizeSpecLabel(s) : "";
          if (cleaned) {
            wowSpec = cleaned;
            source = source || "classic-armory";
    ...
    if (armoryPayloadForGear) {
      const g = gearScoreFromClassicArmoryCharacter(armoryPayloadForGear);
      if (g != null) {
        gearScore = g;
    ...
    if (includeClassicArmoryGearScore && gearScore == null && !armoryPayloadForGear) {
      try {
        const extra = await fetchClassicArmoryCharacter({
          baseUrl: classicArmoryApiBase,
          region: raiderIoRegion || blizzardRegion || "eu",
          flavor: classicArmoryFlavor,
          realmSlug: slug,
        ...
        if (extra) {
          const g = gearScoreFromClassicArmoryCharacter(extra);
          if (g != null) {
            gearScore = g;
            gearScoreSource = "classic-armory";
          }
    ...
    return { wowClass, wowSpec, source, gearScore, gearScoreSource };
  };
}
