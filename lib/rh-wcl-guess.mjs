...
/** @typedef {{ raidHelperName: string, wclCharacterNames: string[], wclSources?: string[], wclGuessConfidence?: number[] }} RhWclLinkRow */
 ...
 * @param {{ minScore?: number, keepEmptyRaidHelperRows?: boolean }} [opts]
 * @returns {{ links: RhWclLinkRow[], stats: { guessedPairs: number, lockedWcl: number, skippedLowScore: number, conflicts: string[] } }}
 */
export function mergeRhWclGuess(existingLinks, raidHelperDisplayNames, wclDisplayNames, opts = {}) {
  ...
  const ensureRow = (rhDisplay) => {
    ...
    let row = byKey.get(k);
    if (!row) {
      row = { raidHelperName: rhDisplay, wclCharacterNames: [], wclSources: [], wclGuessConfidence: [] };
      byKey.set(k, row);
    }
    return row;
  };
