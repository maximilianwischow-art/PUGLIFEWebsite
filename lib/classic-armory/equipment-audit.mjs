...
/** Compact enchant + gem counts for roster cards and summaries. */
export function summarizeGearAudit(audit) {
  const s = audit?.summary && typeof audit.summary === "object" ? audit.summary : {};
  const g = s.gemQualityCounts && typeof s.gemQualityCounts === "object" ? s.gemQualityCounts : {};
  return {
    ok: Boolean(s.ok),
    missingEnchants: Number(s.missingEnchants) || 0,
    emptySockets: Number(s.emptySockets) || 0,
    gems: {
      epic: Number(g.purple) || 0,
      rare: Number(g.blue) || 0,
      uncommon: Number(g.green) || 0,
      unknown: Number(g.unknown) || 0,
    },
  };
}

/** Plain-text fallback (API logs, etc.). UI uses `public/gear-audit-display.js`. */
export function formatGearAuditSummaryLine(summary) {
  ...
  const enc = missingEnc > 0 ? `${missingEnc} enc` : "Enc OK";
  const gemParts = [];
  if (emptySock > 0) gemParts.push(`${emptySock} miss`);
  if (epic > 0) gemParts.push(String(epic));
  if (rare > 0) gemParts.push(String(rare));
  if (uncommon > 0) gemParts.push(String(uncommon));
  if (unknown > 0) gemParts.push(`?${unknown}`);
  const gems = gemParts.length ? gemParts.join("/") : "gems OK";
  const warn = missingEnc > 0 || emptySock > 0;
  const title = [
    ...
    .filter(Boolean)
    .join(" · ");
  return { short: `${enc} · ${gems}`, title, warn };
}
