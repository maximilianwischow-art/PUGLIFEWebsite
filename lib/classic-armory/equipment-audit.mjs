...
function buildEnchantSlots(slots) {
  const byId = new Map(
    (Array.isArray(slots) ? slots : [])
      .filter((s) => TBC_ENCHANTABLE_SLOTS.has(String(s?.slotId || "").toUpperCase()))
...
export function parseClassicArmoryEquipmentAudit(input) {
  ...
  for (const row of equipment) {
    ...
    const issues = issuesForSlot(slotType, row);
  ...
  const enchantSlots = buildEnchantSlots(slots);

  return {
    characterName,
    className: String(character?.class_name || "").trim() || null,
    armoryUrl,
