 ...
 *   - Heart of Darkness needs per Discord user (`hod_needs_*` tables)
...
CREATE TABLE IF NOT EXISTS nv_needs_current_items (
  ...
  request_character_name TEXT NOT NULL DEFAULT '',
  request_character_role TEXT NOT NULL DEFAULT 'main',
...
-- ============================================================
-- Heart of Darkness needs (per Discord user, Phase 3 prep)
-- ============================================================
CREATE TABLE IF NOT EXISTS hod_needs_current (
  user_id TEXT PRIMARY KEY,
  display_name TEXT NOT NULL,
  needed_count INTEGER NOT NULL DEFAULT 0,
  updated_at INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS hod_needs_current_items (
  user_id TEXT NOT NULL,
  position INTEGER NOT NULL,
  item_id INTEGER NOT NULL DEFAULT 0,
  item_name TEXT NOT NULL DEFAULT '',
  profession TEXT NOT NULL DEFAULT '',
  hod_needed INTEGER NOT NULL DEFAULT 1,
  request_character_name TEXT NOT NULL DEFAULT '',
  request_character_role TEXT NOT NULL DEFAULT 'main',
  PRIMARY KEY (user_id, position),
  FOREIGN KEY (user_id) REFERENCES hod_needs_current(user_id) ON DELETE CASCADE
);

CREATE TABLE IF NOT EXISTS hod_needs_history (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id TEXT NOT NULL,
  display_name TEXT NOT NULL,
  action TEXT NOT NULL CHECK (action IN ('upsert', 'delete', 'migrate')),
...
CREATE INDEX IF NOT EXISTS idx_hod_history_user
  ON hod_needs_history(user_id, submitted_at DESC);

CREATE INDEX IF NOT EXISTS idx_hod_history_time
  ON hod_needs_history(submitted_at DESC);

CREATE TABLE IF NOT EXISTS hod_needs_history_items (
  history_id INTEGER NOT NULL,
  position INTEGER NOT NULL,
  item_id INTEGER NOT NULL DEFAULT 0,
  item_name TEXT NOT NULL DEFAULT '',
  ...
  hod_needed INTEGER NOT NULL DEFAULT 1,
  PRIMARY KEY (history_id, position),
  FOREIGN KEY (history_id) REFERENCES hod_needs_history(id) ON DELETE CASCADE
);
...
-- ============================================================
-- World of Warcraft: Forever (Classic+) character picks
-- One planned roll per Discord account for The WoW Forever Squad.
-- ============================================================
CREATE TABLE IF NOT EXISTS wow_forever_picks (
  user_id TEXT PRIMARY KEY,
  display_name TEXT NOT NULL DEFAULT '',
...
CREATE INDEX IF NOT EXISTS idx_wow_forever_picks_faction
  ON wow_forever_picks(faction, class, updated_at DESC);

-- Shared 20-man Forever raid roster (slot board). One document row.
CREATE TABLE IF NOT EXISTS wow_forever_raid_roster (
...
INSERT OR IGNORE INTO wow_forever_raid_roster (id, slots_json, updated_at, updated_by)
VALUES (1, '[]', 0, '');
...
CREATE INDEX IF NOT EXISTS idx_identity_proposals_status
  ON identity_proposals(status, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_identity_proposals_discord
  ON identity_proposals(discord_user_id) WHERE discord_user_id IS NOT NULL;
...
-- MVP-awarded winners. One row per (closed-round, canonical user). Materialised
-- from mvp_votes whenever votes change so the leaderboard can resolve the
-- "MVP hall of fame" achievement badge without ever calling the live HoF
-- pipeline. user_id is nullable because some legacy winner names may not
-- yet resolve to a canonical users row; the leaderboard checks userId, but
-- character_name remains useful for diagnostics / future name-based matching.
CREATE TABLE IF NOT EXISTS mvp_awards (
  round_key TEXT NOT NULL,
  user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
  character_name TEXT NOT NULL,
  raid_code TEXT,
  raid_start_time INTEGER,
  winner_votes INTEGER NOT NULL DEFAULT 0,
  awarded_at INTEGER NOT NULL,
  PRIMARY KEY (round_key)
);
CREATE INDEX IF NOT EXISTS idx_mvp_awards_user ON mvp_awards(user_id);
CREATE INDEX IF NOT EXISTS idx_mvp_awards_name ON mvp_awards(character_name);
...
CREATE TABLE IF NOT EXISTS latest_raid_parse_summary (
  user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  character_id INTEGER REFERENCES user_characters(id) ON DELETE SET NULL,
  character_name TEXT NOT NULL DEFAULT '',
...
function applyAdditiveColumnMigrations(db) {
  const additions = [
    ...
    { table: "user_characters", column: "gear_score", type: "REAL" },
    { table: "user_characters", column: "gear_score_at", type: "INTEGER" },
    { table: "user_characters", column: "gear_score_source", type: "TEXT" },
    { table: "nv_needs_current", column: "request_character_name", type: "TEXT NOT NULL DEFAULT ''" },
    { table: "nv_needs_current", column: "request_character_role", type: "TEXT NOT NULL DEFAULT 'main'" },
    { table: "nv_needs_history", column: "request_character_name", type: "TEXT NOT NULL DEFAULT ''" },
    { table: "nv_needs_history", column: "request_character_role", type: "TEXT NOT NULL DEFAULT 'main'" },
    // Per-item character tags let a single Discord user hold separate craftable
    // lists per character (Main vs Alt) inside one submission — see nvUpsertCurrent.
    { table: "nv_needs_current_items", column: "request_character_name", type: "TEXT NOT NULL DEFAULT ''" },
    { table: "nv_needs_current_items", column: "request_character_role", type: "TEXT NOT NULL DEFAULT 'main'" },
    { table: "wow_forever_picks", column: "role", type: "TEXT NOT NULL DEFAULT ''" },
    { table: "wow_forever_picks", column: "spec", type: "TEXT NOT NULL DEFAULT ''" },
  ];
  ...
  // One-shot backfill: legacy item rows predate the per-item character tag, so
  // inherit the character from their parent submission header. Guarded by a
  // schema_meta flag so it never rewrites already-tagged rows.
  try {
    const flag = db.prepare(`SELECT value FROM schema_meta WHERE key = ?`).get("nv_items_char_backfilled");
    if (flag?.value !== "1") {
      db.exec(`
        UPDATE nv_needs_current_items
        SET request_character_name = COALESCE(
              (SELECT h.request_character_name FROM nv_needs_current h WHERE h.user_id = nv_needs_current_items.user_id),
              ''
            ),
            request_character_role = COALESCE(
              (SELECT h.request_character_role FROM nv_needs_current h WHERE h.user_id = nv_needs_current_items.user_id),
              'main'
            )
        WHERE request_character_name IS NULL OR request_character_name = '';
      `);
      db.prepare(`INSERT INTO schema_meta (key, value) VALUES ('nv_items_char_backfilled', '1')
                  ON CONFLICT(key) DO UPDATE SET value = '1'`).run();
    }
  } catch (error) {
    console.warn("[item-needs-db] nv item character backfill failed:", error?.message || error);
  }
  try {
    db.exec(`
      CREATE TABLE IF NOT EXISTS identity_proposals (
    ...
    console.warn("[item-needs-db] identity_proposals migration failed:", error?.message || error);
  }
}
 ...
 *   requestCharacterRole?: 'main' | 'alt' | string,
 * }} input
 * @returns {{ historyId: number, action: 'upsert'|'delete', items: ReturnType<typeof sanitizeItemRow>[] }}
 */
export function sanitizeNvRequestCharacterRole(raw) {
  const role = String(raw || "").trim().toLowerCase();
  return role === "alt" ? "alt" : "main";
}

export function sanitizeNvRequestCharacterName(raw) {
  return String(raw || "")
    .trim()
    .replace(/\s+/g, " ")
    .slice(0, 64);
...
/** Normalized (case-insensitive) key used to group a user's items per character. */
export function nvRequestCharacterKey(raw) {
  return sanitizeNvRequestCharacterName(raw).toLowerCase();
}

/**
 * Stable identifier for one craftable list = a Discord user's submission for a
 * single character. `main` and `alt` (or any two distinct character names) each
 * get their own entry so the same item can be reserved on both.
 */
export function nvEntryKey(userId, requestCharacterName) {
  return `${String(userId || "").trim()}::${nvRequestCharacterKey(requestCharacterName)}`;
}

/**
 * Replace the current craftable list **for one character** of a Discord user,
 * leaving that user's other characters' lists untouched. All of the user's item
 * rows live in `nv_needs_current_items` tagged with `request_character_name`;
 * this call rewrites only the rows whose character key matches
 * `requestCharacterName`, then re-numbers positions so the (user_id, position)
 * primary key stays intact. Pass `items: []` to clear just that character.
 */
export function nvUpsertCurrent({
  userId,
  displayName,
  items,
  neededCount = 0,
  ...
  const charName = sanitizeNvRequestCharacterName(requestCharacterName);
  const charRole = sanitizeNvRequestCharacterRole(requestCharacterRole);
  const charKey = charName.toLowerCase();

  const result = db().transaction(() => {
    // Keep this user's rows for *other* characters; only the target character's
    // rows are replaced by the incoming list.
    const existing = db()
      .prepare(
        `SELECT item_id AS itemID, item_name AS itemName, profession,
                vortex_needed AS vortexNeeded,
                request_character_name AS requestCharacterName,
                request_character_role AS requestCharacterRole
    ...
    const kept = existing.filter(
      (r) => sanitizeNvRequestCharacterName(r.requestCharacterName).toLowerCase() !== charKey
    );
    const incoming = sanitized.map((row) => ({
      itemID: row.itemID,
      itemName: row.itemName,
      profession: row.profession,
      vortexNeeded: row.vortexNeeded,
      requestCharacterName: charName,
      requestCharacterRole: charRole,
    }));
    ...
    if (combined.length === 0) {
      ...
      db()
        .prepare(
          `INSERT INTO nv_needs_current
             (user_id, display_name, needed_count, updated_at, request_character_name, request_character_role)
           VALUES (?, ?, ?, ?, ?, ?)
           ON CONFLICT(user_id) DO UPDATE SET
             display_name = excluded.display_name,
             needed_count = excluded.needed_count,
             updated_at = excluded.updated_at,
             request_character_name = excluded.request_character_name,
             request_character_role = excluded.request_character_role`
        )
        .run(uid, dn, Math.max(0, Math.floor(Number(neededCount) || 0)), now, charName, charRole);
      const insertItem = db().prepare(
        `INSERT INTO nv_needs_current_items
           (user_id, position, item_id, item_name, profession, vortex_needed, request_character_name, request_character_role)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
      );
      combined.forEach((row, idx) => {
        insertItem.run(
          ...
          row.profession,
          row.vortexNeeded,
          sanitizeNvRequestCharacterName(row.requestCharacterName),
    ...
    const histInfo = db()
      .prepare(
        `INSERT INTO nv_needs_history
           (user_id, display_name, action, needed_count, submitted_at, request_character_name, request_character_role)
         VALUES (?, ?, ?, ?, ?, ?, ?)`
      )
      .run(uid, dn, action, Math.max(0, Math.floor(Number(neededCount) || 0)), now, charName, charRole);
...
/**
 * Clear one character's craftable list (or the legacy unnamed list) while still
 * appending a `delete` history row. Pass `requestCharacterName` to target a
 * specific character; omit it to clear the empty-named legacy entry.
 */
export function nvDeleteCurrent({ userId, displayName, updatedAt, requestCharacterName = "" }) {
  return nvUpsertCurrent({ userId, displayName, items: [], updatedAt, requestCharacterName });
}

/**
 * Read all current craftable lists as one entry **per (user, character)**,
 * ordered most recently updated first. Items are grouped by their per-item
 * character tag so a single Discord user can appear once per character (Main
 * and Alt), each carrying its own item list and a stable `entryKey`.
 */
export function nvGetAllCurrent() {
  ...
  const headerByUser = new Map(headers.map((h) => [h.userId, h]));
  const itemRows = db()
    .prepare(
      `SELECT user_id AS userId,
              position,
  ...
  // Group items by user + normalized character key, preserving first-seen order.
  const groups = new Map();
  for (const it of itemRows) {
    const header = headerByUser.get(it.userId);
    if (!header) continue;
    const charName = sanitizeNvRequestCharacterName(it.requestCharacterName);
    const charKey = charName.toLowerCase();
    const key = `${it.userId}::${charKey}`;
    ...
    if (!group) {
      group = {
        ...
        neededCount: Number(header.neededCount) || 0,
        updatedAt: Number(header.updatedAt) || 0,
        requestCharacterName: charName,
        requestCharacterRole: sanitizeNvRequestCharacterRole(it.requestCharacterRole),
        items: [],
      };
    ...
    group.items.push({
      ...
      vortexNeeded: it.vortexNeeded,
    });
  }

  return [...groups.values()].sort((a, b) => Number(b.updatedAt || 0) - Number(a.updatedAt || 0));
}
...
export function nvGetHistory({ limit = 200, userId } = {}) {
    ...
    ? db()
        .prepare(
          `SELECT id, user_id AS userId, display_name AS displayName, action, needed_count AS neededCount, submitted_at AS submittedAt,
                  request_character_name AS requestCharacterName, request_character_role AS requestCharacterRole
  ...
  return rows.map((r) => ({
    ...
    items: itemsByHistory.get(r.id) || [],
  }));
}

// ============================================================
// Heart of Darkness needs — public API
// ============================================================

function sanitizeHodItemRow(row) {
  if (!row || typeof row !== "object") return null;
  const itemID = Number(row.itemID || row.itemId || 0);
  const itemName = String(row.itemName || row.name || "").trim();
  const profession = String(row.profession || "").trim();
  const hodRaw = Number(row.hodNeeded || row.hodCount || row.count || 1);
  const hodNeeded = Number.isFinite(hodRaw) ? Math.max(1, Math.min(20, Math.floor(hodRaw))) : 1;
  const safeId = Number.isFinite(itemID) ? Math.max(0, Math.floor(itemID)) : 0;
  if (!itemName && safeId <= 0) return null;
  return { itemID: safeId, itemName, profession, hodNeeded };
}

function sanitizeHodItemsArray(items) {
  return (Array.isArray(items) ? items : []).map(sanitizeHodItemRow).filter(Boolean).slice(0, 30);
}

export const hodEntryKey = nvEntryKey;

export function hodUpsertCurrent({
  userId,
  displayName,
  items,
  neededCount = 0,
  ...
  const uid = String(userId || "").trim();
  if (!uid) throw new Error("hodUpsertCurrent requires a non-empty userId");
  const dn = String(displayName || "").trim() || "Unknown";
  const now = Number.isFinite(Number(updatedAt)) ? Math.floor(Number(updatedAt)) : Date.now();
  const sanitized = sanitizeHodItemsArray(items);
  const action = sanitized.length === 0 ? "delete" : "upsert";
  const charName = sanitizeNvRequestCharacterName(requestCharacterName);
  const charRole = sanitizeNvRequestCharacterRole(requestCharacterRole);
  const charKey = charName.toLowerCase();
  ...
  const result = db().transaction(() => {
    const existing = db()
      .prepare(
        `SELECT item_id AS itemID, item_name AS itemName, profession,
                hod_needed AS hodNeeded,
                request_character_name AS requestCharacterName,
                request_character_role AS requestCharacterRole
         FROM hod_needs_current_items
         WHERE user_id = ?
         ORDER BY position`
      )
      .all(uid);
    ...
    const incoming = sanitized.map((row) => ({
      ...
      itemName: row.itemName,
      profession: row.profession,
      hodNeeded: row.hodNeeded,
      requestCharacterName: charName,
      requestCharacterRole: charRole,
    }));
    const combined = [...kept, ...incoming];

    db().prepare(`DELETE FROM hod_needs_current_items WHERE user_id = ?`).run(uid);
    if (combined.length === 0) {
      db().prepare(`DELETE FROM hod_needs_current WHERE user_id = ?`).run(uid);
    } else {
      db()
        .prepare(
          `INSERT INTO hod_needs_current
             (user_id, display_name, needed_count, updated_at, request_character_name, request_character_role)
           VALUES (?, ?, ?, ?, ?, ?)
           ON CONFLICT(user_id) DO UPDATE SET
             display_name = excluded.display_name,
        ...
        .run(uid, dn, Math.max(0, Math.floor(Number(neededCount) || 0)), now, charName, charRole);
      const insertItem = db().prepare(
        `INSERT INTO hod_needs_current_items
           (user_id, position, item_id, item_name, profession, hod_needed, request_character_name, request_character_role)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
      );
      combined.forEach((row, idx) => {
        insertItem.run(
    ...
    if (sanitized.length) {
      const insertHistItem = db().prepare(
        `INSERT INTO hod_needs_history_items
           (history_id, position, item_id, item_name, profession, hod_needed)
         VALUES (?, ?, ?, ?, ?, ?)`
      );
      sanitized.forEach((row, idx) => {
        insertHistItem.run(historyId, idx, row.itemID, row.itemName, row.profession, row.hodNeeded);
      });
    }
    return { historyId, action, items: sanitized };
  })();
...
export function hodDeleteCurrent({ userId, displayName, updatedAt, requestCharacterName = "" }) {
  return hodUpsertCurrent({ userId, displayName, items: [], updatedAt, requestCharacterName });
}

export function hodGetAllCurrent() {
  const headers = db()
    .prepare(
      `SELECT user_id AS userId,
              display_name AS displayName,
...
export function hodGetHistory({ limit = 200, userId } = {}) {
  const cap = Math.max(1, Math.min(2000, Math.floor(Number(limit) || 200)));
  const rows = userId
    ? db()
        .prepare(
          `SELECT id, user_id AS userId, display_name AS displayName, action, needed_count AS neededCount, submitted_at AS submittedAt,
                  request_character_name AS requestCharacterName, request_character_role AS requestCharacterRole
           FROM hod_needs_history WHERE user_id = ?
           ORDER BY submitted_at DESC, id DESC LIMIT ?`
        )
        .all(String(userId), cap)
    : db()
  ...
  const itemRows = db()
    .prepare(
      `SELECT history_id AS historyId, item_id AS itemID, item_name AS itemName, profession, hod_needed AS hodNeeded, position
       FROM hod_needs_history_items
       WHERE history_id IN (${placeholders})
       ORDER BY history_id, position`
    )
    .all(...ids);
  ...
  for (const it of itemRows) {
    const arr = itemsByHistory.get(it.historyId) || [];
    arr.push({ itemID: it.itemID, itemName: it.itemName, profession: it.profession, hodNeeded: it.hodNeeded });
    itemsByHistory.set(it.historyId, arr);
  }
  return rows.map((r) => ({
    ...r,
...
function wowForeverRowToPublic(row) {
  if (!row) return null;
  return {
    ...
    specId: String(row.spec || ""),
    updatedAt: Number(row.updated_at) || 0,
  };
}

export function wowForeverGetByUserId(userId) {
  const uid = String(userId || "").trim();
  if (!uid) return null;
  const row = db().prepare(`SELECT * FROM wow_forever_picks WHERE user_id = ?`).get(uid);
  return wowForeverRowToPublic(row);
}

export function wowForeverListAll() {
  const rows = db()
    .prepare(
      `SELECT * FROM wow_forever_picks
       ORDER BY faction ASC, class ASC, updated_at DESC`
    )
    .all();
  return rows.map(wowForeverRowToPublic);
}

export function wowForeverUpsert({
  userId,
  displayName,
  avatar,
  characterName,
  ...
  const uid = String(userId || "").trim();
  if (!uid) throw new Error("wowForeverUpsert requires userId");
  const now = Number.isFinite(Number(updatedAt)) ? Math.floor(Number(updatedAt)) : Date.now();
  db()
    .prepare(
      `INSERT INTO wow_forever_picks
         (user_id, display_name, avatar, character_name, race, faction, gender, class, role, spec, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(user_id) DO UPDATE SET
         display_name = COALESCE(NULLIF(excluded.display_name, ''), wow_forever_picks.display_name),
  ...
  return wowForeverGetByUserId(uid);
}

export function wowForeverDeleteByUserId(userId) {
  const uid = String(userId || "").trim();
  if (!uid) return false;
  const info = db().prepare(`DELETE FROM wow_forever_picks WHERE user_id = ?`).run(uid);
  if (Number(info?.changes) > 0) {
    wowForeverRaidRosterRemoveUser(uid);
  }
  return Number(info?.changes) > 0;
}

const WOW_FOREVER_RAID_SLOTS = 20;

function normalizeRaidRosterSlots(raw) {
  const out = Array.from({ length: WOW_FOREVER_RAID_SLOTS }, () => "");
  if (!Array.isArray(raw)) return out;
  for (let i = 0; i < WOW_FOREVER_RAID_SLOTS; i += 1) {
    out[i] = String(raw[i] || "").trim();
  }
  return out;
}

function parseRaidRosterSlotsJson(json) {
  try {
    return normalizeRaidRosterSlots(JSON.parse(String(json || "[]")));
  } catch {
    return normalizeRaidRosterSlots([]);
  }
}

export function wowForeverRaidRosterGet() {
  const row = db().prepare(`SELECT * FROM wow_forever_raid_roster WHERE id = 1`).get();
  if (!row) {
    return {
      slots: normalizeRaidRosterSlots([]),
      updatedAt: 0,
      updatedBy: "",
      slotCount: WOW_FOREVER_RAID_SLOTS,
    };
  }
  return {
    slots: parseRaidRosterSlotsJson(row.slots_json),
    updatedAt: Number(row.updated_at) || 0,
...
export function wowForeverRaidRosterSave(slots, updatedBy) {
  const normalized = normalizeRaidRosterSlots(slots);
  const seen = new Set();
  for (let i = 0; i < normalized.length; i += 1) {
    const uid = normalized[i];
    if (!uid) continue;
    if (seen.has(uid)) {
      throw new Error("Each raider can only occupy one raid slot");
    }
    seen.add(uid);
  }
  const now = Date.now();
  ...
  db()
    .prepare(
      `INSERT INTO wow_forever_raid_roster (id, slots_json, updated_at, updated_by)
       VALUES (1, ?, ?, ?)
    ...
    .run(JSON.stringify(normalized), now, by);
  return wowForeverRaidRosterGet();
}

export function wowForeverRaidRosterRemoveUser(userId) {
  const uid = String(userId || "").trim();
  if (!uid) return wowForeverRaidRosterGet();
  const current = wowForeverRaidRosterGet();
  if (!current.slots.includes(uid)) return current;
  const next = current.slots.map((slot) => (slot === uid ? "" : slot));
  return wowForeverRaidRosterSave(next, current.updatedBy || "");
}
...
function characterRowToPublic(row) {
  if (!row) return null;
  const gs = row.gear_score != null ? Number(row.gear_score) : null;
  return {
    id: Number(row.id),
    ...
    gearScoreAt: row.gear_score_at != null ? Number(row.gear_score_at) : null,
    gearScoreSource: row.gear_score_source || null,
...
/** Update an existing canonical user by internal id. Used by admin identity
 * tooling where the row is already selected and the Discord ID itself may be
 * changing. */
export function userUpdateById({ userId, discordUserId, raidHelperName, displayName, guildRole, source = "", updatedAt } = {}) {
  const id = Number(userId);
  if (!Number.isInteger(id) || id <= 0) throw new Error("userUpdateById requires a positive integer userId");
  const now = Number.isFinite(Number(updatedAt)) ? Math.floor(Number(updatedAt)) : Date.now();
  const discordId = discordUserId != null && String(discordUserId).trim() ? String(discordUserId).trim() : null;
  const rhName = raidHelperName != null ? String(raidHelperName).trim() || null : undefined;
  const rhKey = rhName !== undefined ? (rhName ? rhNameKey(rhName) || null : null) : undefined;
  const display = displayName != null ? String(displayName).trim() || null : undefined;
  const role = guildRole != null ? String(guildRole).trim() || null : undefined;
  return db().transaction(() => {
    const row = db().prepare(`SELECT * FROM users WHERE id = ?`).get(id);
    if (!row) throw new Error("User not found");
    if (discordId && row.discord_user_id !== discordId) {
      const duplicate = db().prepare(`SELECT * FROM users WHERE discord_user_id = ? AND id <> ?`).get(discordId, id);
      if (duplicate) throw new Error(`Discord ID already belongs to user #${duplicate.id}`);
    }
    const updates = [];
    const params = [];
    if (discordUserId !== undefined && (row.discord_user_id || null) !== discordId) {
      updates.push("discord_user_id = ?");
      params.push(discordId);
    }
    if (rhName !== undefined && (row.raid_helper_name || null) !== rhName) {
      updates.push("raid_helper_name = ?", "raid_helper_name_key = ?");
      params.push(rhName, rhKey);
    }
    if (display !== undefined && (row.display_name || null) !== display) {
      updates.push("display_name = ?");
      params.push(display);
    }
    if (role !== undefined && (row.guild_role || null) !== role) {
      updates.push("guild_role = ?");
      params.push(role);
    }
    updates.push("last_seen_at = ?");
    ...
    appendUserHistory({ userId: id, action: "update", source, row: updated, when: now });
    return userRowToPublic(updated);
  })();
}
...
export function characterUpsert(input) {
  ...
  let gearScoreWrite = null;
  let gearScoreSourceWrite = null;
  if (input?.gearScore != null) {
    const gn = Number(input.gearScore);
    if (Number.isFinite(gn) && gn > 0) {
      gearScoreWrite = Math.round(gn);
      gearScoreSourceWrite = String(input?.gearScoreSource || "classic-armory").trim() || "classic-armory";
    }
  }
  ...
  return db().transaction(() => {
    ...
    if (!existing) {
      const info = db()
        .prepare(
          `INSERT INTO user_characters
             (user_id, character_name, character_name_key, wow_class, wow_spec, realm,
              gear_score, gear_score_at, gear_score_source,
        ...
        .run(
          ...
          realm,
          gearScoreWrite,
          gearScoreWrite != null ? now : null,
          gearScoreSourceWrite,
          isMain ? 1 : 0,
          discoveredVia,
          now,
          now
    ...
    if (gearScoreWrite != null) {
      const prevGs = existing.gear_score != null ? Math.round(Number(existing.gear_score)) : null;
      if (prevGs !== gearScoreWrite) {
        updates.push("gear_score = ?");
        params.push(gearScoreWrite);
        updates.push("gear_score_at = ?");
        params.push(now);
        updates.push("gear_score_source = ?");
        params.push(gearScoreSourceWrite);
      }
    }
    updates.push("last_seen_at = ?");
    params.push(now);
...
/**
 * Every character row across every user, ordered by id. Pass
 * `missingClassOrSpec: true` to restrict to rows where either
 * `wow_class` or `wow_spec` is empty/null — used by the
 * `runSyncCharacterSpecs` worker to avoid hammering external APIs for
 * rows we already have data for.
 *
 * `forCharacterSpecsSync: true` selects rows missing class/spec **or**
 * missing persisted Classic Armory GearScore (`gear_score` null) — useful
 * for one-shot CLI backfills (`scripts/backfill-character-specs.mjs`).
 *
 * `missingGearScoreOnly: true` selects rows where `gear_score` is null
 * (Classic Armory gear sync worker).
 */
export function charactersListAll({
  missingClassOrSpec = false,
  forCharacterSpecsSync = false,
  missingGearScoreOnly = false,
} = {}) {
  let where = "";
  if (missingGearScoreOnly) {
    where = "WHERE gear_score IS NULL";
  } else if (forCharacterSpecsSync) {
    where = `WHERE wow_class IS NULL
         OR TRIM(COALESCE(wow_class, '')) = ''
         OR wow_spec IS NULL
         OR TRIM(COALESCE(wow_spec, '')) = ''
         OR gear_score IS NULL`;
  } else if (missingClassOrSpec) {
    where = `WHERE wow_class IS NULL
         OR TRIM(COALESCE(wow_class, '')) = ''
         OR wow_spec IS NULL
         OR TRIM(COALESCE(wow_spec, '')) = ''`;
  ...
  const rows = db()
    .prepare(`SELECT * FROM user_characters ${where} ORDER BY id`)
    .all();
  return rows.map(characterRowToPublic);
}

/** Find every owner of a normalised character name. More than one row means
 * the identity store needs admin cleanup before the name can be canonical. */
export function characterOwnersByName(characterName) {
  const key = rhNameKey(characterName);
  if (!key) return [];
  const rows = db()
    .prepare(
      `SELECT c.*, u.discord_user_id, u.raid_helper_name, u.display_name
         FROM user_characters c
         INNER JOIN users u ON u.id = c.user_id
        WHERE c.character_name_key = ?
        ORDER BY c.is_main DESC, c.last_seen_at DESC, c.id ASC`
    )
    .all(key);
  return rows.map((row) => ({
    ...characterRowToPublic(row),
    owner: {
...
/** Move a character row to another canonical user, preserving history. If the
 * target already owns the same character key, the source row is removed. */
export function characterMoveToUser({ characterId, targetUserId, source = "", updatedAt } = {}) {
  const cid = Number(characterId);
  const targetId = Number(targetUserId);
  if (!Number.isInteger(cid) || cid <= 0) throw new Error("characterMoveToUser: bad characterId");
  if (!Number.isInteger(targetId) || targetId <= 0) throw new Error("characterMoveToUser: bad targetUserId");
  const now = Number.isFinite(Number(updatedAt)) ? Math.floor(Number(updatedAt)) : Date.now();
  return db().transaction(() => {
    const charRow = db().prepare(`SELECT * FROM user_characters WHERE id = ?`).get(cid);
    ...
    const duplicate = db()
      .prepare(`SELECT * FROM user_characters WHERE user_id = ? AND character_name_key = ? AND id <> ?`)
      .get(targetId, charRow.character_name_key, cid);
    if (duplicate) {
      db().prepare(`DELETE FROM user_characters WHERE id = ?`).run(cid);
      appendCharacterHistory({
        characterId: cid,
        userId: Number(charRow.user_id),
        action: "remove",
        source,
        row: charRow,
        when: now,
      });
      return characterRowToPublic(duplicate);
    }
    db().prepare(`UPDATE user_characters SET user_id = ?, is_main = 0, last_seen_at = ? WHERE id = ?`).run(targetId, now, cid);
    const moved = db().prepare(`SELECT * FROM user_characters WHERE id = ?`).get(cid);
    appendCharacterHistory({ characterId: cid, userId: targetId, action: "update", source, row: moved, when: now });
    return characterRowToPublic(moved);
  })();
}

/** Replace a user's accepted character list exactly. Existing rows not in the
 * desired list are removed; desired rows are upserted. */
export function userReplaceCharacters({ userId, characters = [], source = "", updatedAt } = {}) {
  const uid = Number(userId);
  if (!Number.isInteger(uid) || uid <= 0) throw new Error("userReplaceCharacters: bad userId");
  const now = Number.isFinite(Number(updatedAt)) ? Math.floor(Number(updatedAt)) : Date.now();
  const desired = [];
  const seen = new Set();
  for (const raw of Array.isArray(characters) ? characters : []) {
    const row = typeof raw === "string" ? { characterName: raw } : raw;
    const characterName = String(row?.characterName || row?.name || "").trim();
    const key = rhNameKey(characterName);
    if (!characterName || !key || seen.has(key)) continue;
    seen.add(key);
    desired.push({
      characterName,
      key,
      wowClass: row?.wowClass ?? row?.className ?? null,
      wowSpec: row?.wowSpec ?? row?.specName ?? null,
      realm: row?.realm ?? null,
      discoveredVia: row?.discoveredVia || "manual",
      isMain: Boolean(row?.isMain),
    });
  }
  return db().transaction(() => {
    const existing = db().prepare(`SELECT * FROM user_characters WHERE user_id = ?`).all(uid);
    const desiredKeys = new Set(desired.map((row) => row.key));
    for (const row of existing) {
      if (desiredKeys.has(row.character_name_key)) continue;
      db().prepare(`DELETE FROM user_characters WHERE id = ?`).run(row.id);
      appendCharacterHistory({
...
/** Merge one canonical user into another. Characters are moved first, then
 * source metadata is retained in history and the source row is deleted. */
export function userMergeInto({ sourceUserId, targetUserId, source = "", updatedAt } = {}) {
  const fromId = Number(sourceUserId);
  const toId = Number(targetUserId);
  if (!Number.isInteger(fromId) || fromId <= 0) throw new Error("userMergeInto: bad sourceUserId");
  if (!Number.isInteger(toId) || toId <= 0) throw new Error("userMergeInto: bad targetUserId");
  if (fromId === toId) throw new Error("Cannot merge a user into itself");
  const now = Number.isFinite(Number(updatedAt)) ? Math.floor(Number(updatedAt)) : Date.now();
  return db().transaction(() => {
    const sourceRow = db().prepare(`SELECT * FROM users WHERE id = ?`).get(fromId);
    const targetRow = db().prepare(`SELECT * FROM users WHERE id = ?`).get(toId);
    if (!sourceRow) throw new Error("Source user not found");
    if (!targetRow) throw new Error("Target user not found");
    const characters = db().prepare(`SELECT * FROM user_characters WHERE user_id = ?`).all(fromId);
    for (const charRow of characters) {
      const duplicate = db()
        .prepare(`SELECT * FROM user_characters WHERE user_id = ? AND character_name_key = ?`)
        .get(toId, charRow.character_name_key);
      if (duplicate) {
        db().prepare(`DELETE FROM user_characters WHERE id = ?`).run(charRow.id);
        appendCharacterHistory({
          characterId: Number(charRow.id),
          userId: fromId,
          action: "remove",
          source,
          row: charRow,
        ...
        db().prepare(`UPDATE user_characters SET user_id = ?, is_main = 0, last_seen_at = ? WHERE id = ?`).run(toId, now, charRow.id);
        const moved = db().prepare(`SELECT * FROM user_characters WHERE id = ?`).get(charRow.id);
        appendCharacterHistory({ characterId: Number(charRow.id), userId: toId, action: "update", source, row: moved, when: now });
      }
    }
    for (const table of ["raid_attendance", "death_totals", "badge_state", "raid_appearances", "latest_raid_parse_summary"]) {
      db().prepare(`UPDATE OR IGNORE ${table} SET user_id = ? WHERE user_id = ?`).run(toId, fromId);
      db().prepare(`DELETE FROM ${table} WHERE user_id = ?`).run(fromId);
    }
    for (const table of ["mvp_awards", "first_clear_participants", "best_time_roster", "loot_awards"]) {
      db().prepare(`UPDATE ${table} SET user_id = ? WHERE user_id = ?`).run(toId, fromId);
    }
    db().prepare(`DELETE FROM users WHERE id = ?`).run(fromId);
    appendUserHistory({ userId: toId, action: "merge", source, row: targetRow, when: now });
    return userRowToPublic(db().prepare(`SELECT * FROM users WHERE id = ?`).get(toId));
  })();
}
...
/**
 * Replace `mvp_awards` with the supplied list of winners (one row per
 * closed round). Idempotent — re-running with the same input produces the
 * same rows but refreshes `awarded_at`. Designed to be called from the
 * voting persist hook + a server-boot backfill, so the leaderboard's
 * "MVP hall of fame" badge can resolve straight from SQLite without
 * touching the live HoF pipeline.
 *
 * @param {{
 *   awards: Array<{
 *     roundKey: string,
 ...
 * }} input
 */
export function mvpAwardsReplaceAll({ awards, when } = {}) {
  const list = Array.isArray(awards) ? awards : [];
  const now = Number.isFinite(Number(when)) ? Math.floor(Number(when)) : Date.now();
  return db().transaction(() => {
    db().prepare(`DELETE FROM mvp_awards`).run();
    if (!list.length) return { rows: 0 };
    const stmt = db().prepare(
      `INSERT INTO mvp_awards
         (round_key, user_id, character_name, raid_code, raid_start_time, winner_votes, awarded_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    let rows = 0;
    for (const a of list) {
      const roundKey = String(a?.roundKey || "").trim();
      const characterName = String(a?.characterName || "").trim();
      if (!roundKey || !characterName) continue;
      const userId = Number.isInteger(Number(a?.userId)) && Number(a.userId) > 0 ? Number(a.userId) : null;
      const raidCode = a?.raidCode ? String(a.raidCode) : null;
      const raidStartTime = Number.isFinite(Number(a?.raidStartTime)) ? Math.floor(Number(a.raidStartTime)) : null;
      const winnerVotes = Math.max(0, Math.floor(Number(a?.winnerVotes) || 0));
      stmt.run(roundKey, userId, characterName, raidCode, raidStartTime, winnerVotes, now);
      rows += 1;
    }
    return { rows };
  })();
...
/** Map of canonical `userId` → number of MVP awards. Used by the leaderboard. */
export function mvpAwardsCountsByUser() {
  const rows = db()
    .prepare(
      `SELECT user_id AS userId, COUNT(*) AS n
         FROM mvp_awards
  ...
  for (const r of rows) {
    ...
    if (Number.isInteger(uid) && uid > 0) out.set(uid, Number(r.n || 0));
  }
  return out;
}

/** Read every MVP award for one canonical user (newest first). */
export function mvpAwardsGetByUserId(userId) {
  const uid = Number(userId);
  if (!Number.isInteger(uid) || uid <= 0) return [];
  return db()
    .prepare(
      `SELECT round_key AS roundKey, user_id AS userId, character_name AS characterName,
              raid_code AS raidCode, raid_start_time AS raidStartTime,
              winner_votes AS winnerVotes, awarded_at AS awardedAt
         FROM mvp_awards
        WHERE user_id = ?
        ORDER BY (CASE WHEN raid_start_time IS NULL THEN 0 ELSE raid_start_time END) DESC`
    )
    .all(uid);
}

/** Read every MVP award row. Used by diagnostics and admin UIs. */
export function mvpAwardsGetAll() {
  return db()
    .prepare(
      `SELECT round_key AS roundKey, user_id AS userId, character_name AS characterName,
              raid_code AS raidCode, raid_start_time AS raidStartTime,
 ...
 *  Unlinked WCL names are auto-adopted (same as `raidAttendanceReplaceForWindow`)
 *  so new raiders are not silently dropped from death rankings. */
export function deathTotalsReplaceForWindow({ windowLabel, rows, when } = {}) {
  ...
  return db().transaction(() => {
    ...
    const adoptedOwnerByNameKey = new Map();
    for (const r of rows) {
      const cn = String(r?.characterName || "").trim();
      const deaths = Math.max(0, Math.floor(Number(r?.deaths) || 0));
      if (!cn || deaths <= 0) continue;
      let owner = resolveOwnerForCharacterName(cn);
      if (!owner?.userId) {
        const cnKey = rhNameKey(cn);
        let adopted = cnKey ? adoptedOwnerByNameKey.get(cnKey) || null : null;
        if (!adopted && cnKey) {
          const seeded = userUpsertWithCharacters({
            raidHelperName: cn,
            displayName: cn,
            guildRole: "Peon",
            characterNames: [cn],
            mainCharacterName: cn,
            discoveredVia: "wcl-deaths",
            source: "sync:deaths:auto-adopt",
            updatedAt: now,
          });
          const userId = Number(seeded?.id || 0);
          if (Number.isInteger(userId) && userId > 0) {
            adopted = { userId };
            adoptedOwnerByNameKey.set(cnKey, adopted);
          }
        }
        owner = adopted || resolveOwnerForCharacterName(cn);
      }
      if (owner?.userId) {
...
export function raidAttendanceReplaceForWindow({ windowLabel, rows, when } = {}) {
  ...
  return db().transaction(() => {
    ...
    for (const r of rows) {
      ...
      let owner = resolveOwnerForCharacterName(cn);
      if (!owner?.userId) {
        let adopted = adoptedOwnerByNameKey.get(cnKey) || null;
        if (!adopted) {
          // Keep leaderboard attendance complete even before Account Assignment
          // links are manually reviewed: if a fresh WCL name has no canonical
          // owner yet, seed a lightweight identity row on-the-fly.
          const seeded = userUpsertWithCharacters({
            raidHelperName: cn,
            displayName: cn,
            guildRole: "Peon",
            characterNames: [cn],
            mainCharacterName: cn,
            discoveredVia: "wcl-attendance",
            source: "sync:attendance:auto-adopt",
            updatedAt: now,
          });
          const userId = Number(seeded?.id || 0);
          if (Number.isInteger(userId) && userId > 0) {
...
/**
 * Replace latest participated-raid highest parse rows. `entries` shape:
 * `[{ userId, characterId, characterName, reportCode, reportStartedAt,
 *     bracket, bestValue, bestEncounter, bestFightId, bestMetric }]`
 */
export function latestRaidParseSummaryReplaceAll({ entries, when } = {}) {
  const now = Number.isFinite(Number(when)) ? Math.floor(Number(when)) : Date.now();
  return db().transaction(() => {
    db().prepare(`DELETE FROM latest_raid_parse_summary`).run();
    if (!Array.isArray(entries) || !entries.length) return { rows: 0 };
    const stmt = db().prepare(
      `INSERT INTO latest_raid_parse_summary
         (user_id, character_id, character_name, report_code, report_started_at,
          bracket, best_value, best_encounter, best_fight_id, best_metric, computed_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(user_id) DO UPDATE SET
         character_id = excluded.character_id,
         character_name = excluded.character_name,
    ...
    for (const e of entries) {
      ...
      const reportCode = String(e?.reportCode || "").trim();
      const bestValue = e?.bestValue != null ? Number(e.bestValue) : null;
      if (!Number.isInteger(userId) || userId <= 0 || !reportCode) continue;
      if (bestValue == null || !Number.isFinite(bestValue) || bestValue <= 0) continue;
      stmt.run(
        userId,
        e?.characterId != null ? Number(e.characterId) : null,
        e?.characterName ? String(e.characterName) : "",
        reportCode,
        e?.reportStartedAt != null ? Number(e.reportStartedAt) : null,
        e?.bracket ? String(e.bracket) : "",
        bestValue,
        e?.bestEncounter ? String(e.bestEncounter) : null,
        e?.bestFightId != null ? Number(e.bestFightId) : null,
        e?.bestMetric ? String(e.bestMetric) : null,
...
export function latestRaidParseSummaryGetAll() {
  return db()
    .prepare(
      `SELECT
         user_id AS userId,
         ...
         bracket,
         best_value AS bestValue,
         best_encounter AS bestEncounter,
         best_fight_id AS bestFightId,
         best_metric AS bestMetric,
         computed_at AS computedAt
       FROM latest_raid_parse_summary`
    )
    .all();
}
...
/** Normalise `raid_appearances.report_started_at` (seconds or ms) to epoch ms. */
export function normaliseRaidAppearanceStartedAtMs(raw) {
...
/** Newest `report_started_at` for one WCL report code, or null when unknown. */
export function raidAppearancesReportStartedAtMs(reportCode) {
  const code = String(reportCode || "").trim();
  if (!code) return null;
  const row = db()
    .prepare(
      `SELECT MAX(report_started_at) AS reportStartedAt
         FROM raid_appearances
        WHERE report_code = ?`
    )
    .get(code);
  return normaliseRaidAppearanceStartedAtMs(row?.reportStartedAt);
}
...
/**
 * Build per-user attendance signals straight from `raid_appearances`,
 * scoped to a curated report set. The result mirrors the shape of
 * `raid_attendance` rows the leaderboard consumes:
 *
 *   { perUser: Map<userId, { raidsAttended, raidsConsidered, attendanceHistory }>,
 *     orderedReportCodes: string[] // newest-first, length === raidsConsidered
 *   }
 *
 * - `reportCodes` (optional): restrict the considered set; when omitted,
 *   every distinct report in `raid_appearances` participates.
 * - `recentLimit` (default 6): caps the window to the N most-recent
 *   reports in the considered set, keeping the same shape the rank-pill
 *   logic expects (`attendanceTierGuildRoleFromRaids` clamps at 6).
 *
 * `attendanceHistory[i] === 1` when the user has a row in
 * `raid_appearances` for `orderedReportCodes[i]`. Index 0 is newest.
 *
 * Used by the leaderboard bundle to align `raidsAttended`,
 * `attendanceHistory`, `attendanceRate` and the Peon/Grunt/Veteran rank
 * pill with the admin Event Management selection that drives the
 * `wclEventCount` ("Events") column.
 */
export function raidAppearancesAttendanceWindowByUser({ reportCodes, recentLimit } = {}) {
  const filter = Array.isArray(reportCodes)
    ? reportCodes.map((x) => String(x || "").trim()).filter(Boolean)
    : null;
  if (filter && filter.length === 0) {
    return { perUser: new Map(), orderedReportCodes: [], latestReport: null };
  }
  const cap = Math.max(
    1,
    Math.min(40, Math.floor(Number(recentLimit) || 6))
  );
  // Collect distinct report codes in the considered set, ordered newest first.
  let codeRows;
  if (filter && filter.length) {
    const placeholders = filter.map(() => "?").join(",");
    codeRows = db()
      .prepare(
        `SELECT report_code AS reportCode,
          ...
          WHERE report_code IN (${placeholders})
          GROUP BY report_code
          ORDER BY (CASE WHEN MAX(report_started_at) IS NULL THEN 0
                         ELSE MAX(report_started_at) END) DESC
          LIMIT ?`
      )
      .all(...filter, cap);
  } else {
  ...
  const orderedReportCodes = codeRows
    .map((r) => String(r?.reportCode || ""))
    .filter(Boolean);
  const latestRow = codeRows[0] || null;
  const latestReport = latestRow
    ? {
        reportCode: String(latestRow.reportCode || "").trim(),
        reportStartedAt: Number(latestRow.reportStartedAt || 0) || null,
      }
    : null;
  if (!orderedReportCodes.length) {
    return { perUser: new Map(), orderedReportCodes: [], latestReport: null };
  }
  const indexByCode = new Map();
  orderedReportCodes.forEach((code, idx) => indexByCode.set(code, idx));
  const placeholders = orderedReportCodes.map(() => "?").join(",");
  const presenceRows = db()
    ...
    .all(...orderedReportCodes);
  /** @type {Map<number, { raidsAttended: number, raidsConsidered: number, attendanceHistory: number[] }>} */
  const perUser = new Map();
  const totalRaids = orderedReportCodes.length;
  for (const row of presenceRows) {
    const uid = Number(row?.userId);
    if (!Number.isInteger(uid) || uid <= 0) continue;
    const idx = indexByCode.get(String(row?.reportCode || ""));
    if (!Number.isInteger(idx)) continue;
    let entry = perUser.get(uid);
    if (!entry) {
      entry = {
        raidsAttended: 0,
    ...
    if (entry.attendanceHistory[idx] !== 1) {
      entry.attendanceHistory[idx] = 1;
      entry.raidsAttended += 1;
    }
  }
  return { perUser, orderedReportCodes, latestReport };
}
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        nvUpsertCurrent({
          ...
          requestCharacterName: entry.requestCharacterName,
          requestCharacterRole: entry.requestCharacterRole,
        });
        db()
